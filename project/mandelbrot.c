...
//
inline int mapImg2CPlane(int row, int col, double* re, double* im) {
...
//
inline int coordToNumSteps(int x, int y) {
  double Cre, Cim;
...
#pragma acc data copy(NumSteps[0:][0:])
  {
  ...
  for (i=0; i<ROWS; i++) {
    for (j=0; j<COLS; j++) {
        ...
        Cre = (rmax - rmin) * (double)i / COLS + rmin;
        Cim = (imin - imax) * (double)j / ROWS + imax;
        double Zre = 0.0, Zim = 0.0;
        int k;
        //  printf("For (%d,%d), c is (%lf,%lf)\n", x,y,Cre,Cim);
  ...
  #pragma acc parallel loop reduction(max:max_step)
  for (i=0; i<ROWS; i++) {
    for (j=0; j<COLS; j++) {
