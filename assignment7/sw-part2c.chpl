...
/* TODO
   copy and paste your solution to
   sw-framework.chpl here, and implement
   the modification in problem 2c
*/
...
proc computeMatrixInParallel() {
  ...
  var HSpace = LocHSpace dmapped Block(boundingBox=LocHSpace, targetLocales=taskLocs);
  ...
  //  on loc do {
      ...
      //
      // PART C: moved to computeChunkRecursive
...
proc computeChunkRecursive(taskLocs, H, pathMatrix, i, end, ChunkReady$) {
    ...
    // PART C: make sure the rows above us have finished
    ...
    // PART C: tell the row below us it can go
    if(i + rowsPerChunk < seq1len) {
        ChunkReady$[i+rowsPerChunk, here.id] = 0;
    }

    // PART C: start the next set of columns on the next local (if we aren't the end)
    if(here.id +1 < numLocales) {
